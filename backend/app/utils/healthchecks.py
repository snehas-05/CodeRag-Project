...
def verify_postgres(max_retries: int = 5, delay: float = 2.0) -> bool:
    ...
    for i in range(max_retries):
        try:
            ...
            logger.warning(f"PostgreSQL: Attempt {i+1}/{max_retries} failed: {e}")
            if i < max_retries - 1:
                time.sleep(delay)
    logger.error("PostgreSQL: Failed to connect after multiple attempts.")
    return False
...
def check_dependencies() -> bool:
    ...
    # We check them all, but PostgreSQL is often the most critical for basic startup
    postgres_ok = verify_postgres()
    chroma_ok = verify_chroma()
    es_ok = verify_elasticsearch()
    
    return all([postgres_ok, chroma_ok, es_ok])
